...
drop table is exists dbo.V_ARA_ACTIVITY_TYPE;
select * into dbo.V_ARA_ACTIVITY_TYPE from openquery(awstepic,'select * from EPERMIT_ONLINE.V_ARA_ACTIVITY_TYPE@LINK_TO_DEPERMIT')

drop table is exists dbo.V_ARA_ACTIVITY;
select * into dbo.V_ARA_ACTIVITY from openquery(awstepic,'select * from EPERMIT_ONLINE.V_ARA_ACTIVITY@LINK_TO_DEPERMIT')

drop table is exists dbo.V_ARA_COMBO;
select * into dbo.V_ARA_COMBO from openquery(awstepic,'select * from EPERMIT_ONLINE.V_ARA_COMBO@LINK_TO_DEPERMIT')

drop table is exists dbo.v_well_driller;
select * into dbo.v_well_driller from openquery(awstepic,'select * from EPERMIT_ONLINE.v_well_driller@LINK_TO_DEPERMIT')

drop table is exists dbo.v_ara_actor;
select * into dbo.v_ara_actor from openquery(awstepic,'select * from EPERMIT_ONLINE.v_ara_actor@LINK_TO_DEPERMIT')
--  select * into dbo.ara_combo from openquery(awstepic,'select * from ara.combo')
 

drop table is exists dbo.ara_activity;
select * into dbo.ara_activity from openquery(awstepic,'select * from ara.activity')


drop table is exists dbo.ara_activity_type;
select * into dbo.ara_activity_type from openquery(awstepic,'select * from ara.activity_type')

drop table is exists dbo.ara_comments;
select * into dbo.ara_comments from openquery(awstepic,'select * from ara.comments')


drop table is exists dbo.wells_well_driller;
select *  into wells_well_driller
  from openquery(awstepic,'select * from wells.well_driller')


drop table is exists dbo.ara_combo;
SELECT * 
INTO dbo.ara_combo
